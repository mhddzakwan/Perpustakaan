...
import org.example.perpustakaan.Decorator.LoggingPrinterDecorator;
import org.example.perpustakaan.Decorator.WatermarkPrinterDecorator;
...
//    @FXML TextField fieldIdMember, fieldName, fieldEmail, fieldContact, fieldDepartment, fieldIdBook, fieldTitle, fieldCategory, fieldAuthor;
    ...
    void reserveBook(ActionEvent event) {
        ...
        Task<Void> printTask = new Task<>() {
            ...
            protected Void call() {
                try {
                    Printer printerCanon = new PrinterRPP();

                    // Tambahkan fitur log
                    Printer printerWithLog = new LoggingPrinterDecorator(printerCanon);

                    // Tambahkan watermark
                    Printer printerWithLogAndWatermark = new WatermarkPrinterDecorator(printerWithLog);

                    // Cetak teks pinjaman
                    Printer printer = new ThermalPrinterAdapter();
                    printer.printText(id_peminjaman, nama, nim, tanggal_peminjaman, tenggat, BookList);
                    printerWithLogAndWatermark.print("Bukti Peminjaman");
